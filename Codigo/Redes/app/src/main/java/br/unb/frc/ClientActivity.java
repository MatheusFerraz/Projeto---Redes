    	...
    	if (resultCode == Activity.RESULT_OK && requestCode == fileRequestID) {
    		...
    		if(targetDir.isFile()) {
    			if(targetDir.canRead()) {
    				...
    				setTargetFileStatus(targetDir.getName() + " selecionado para envio.");
    			}
    			else {
    				filePathProvided = false;
    				setTargetFileStatus("Você não possui permissão para ler o arquivo " + targetDir.getName());
    			}
    		}
    		else {
				filePathProvided = false;
    			setTargetFileStatus("Você não pode transferir um diretório. Escolha um único arquivo.");
    		}
        }
    }
